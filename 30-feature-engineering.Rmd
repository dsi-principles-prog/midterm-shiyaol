...
```{r test_scale_function, message = FALSE, error=TRUE}
...
**Why are we doing this**: We can use the package "wru" to guess the race by our surnames.  It will return us the probability for each race and we can take the one with biggest probability as the race.  The raccial discrimination issue is always existing.  So passengers of different race might get different degree of help in the disaster, so it might contribute to the survival status of passengers.
